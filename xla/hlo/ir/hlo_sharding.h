...
class HloSharding {
 public:
  ...
  class IotaTileAssignment {
   public:
    // Creates an IotaTileAssignment canonicalizing `transpose_dims` and
    // `transpose_minor_to_major`.
    static IotaTileAssignment Create(
    ...
    bool operator==(const IotaTileAssignment& other) const {
      return dims() == other.dims() &&
             transpose_dims() == other.transpose_dims() &&
             transpose_minor_to_major() == other.transpose_minor_to_major();
    }
    ...
    absl::Span<const int64_t> transpose_dims() const {
      return absl::MakeSpan(transpose_dims_, transpose_ndims_);
    }

    absl::Span<const int> transpose_minor_to_major() const {
      return absl::MakeSpan(transpose_minor_to_major_, transpose_ndims_);
    }
   ...
   private:
    ...
    static constexpr int kPerTransposeDimBytes = sizeof(int64_t) + sizeof(int);
    static constexpr int kInlinedBytes =
        kInlinedDims * (kPerDimBytes + kPerTransposeDimBytes);
    union Storage {
      char inlined[kInlinedBytes];
      void* out_of_line;
    };
    ...
    IotaTileAssignment(int ndims, int transpose_ndims);
    ...
    int size_bytes() const {
      return ndims_ * kPerDimBytes + transpose_ndims_ * kPerTransposeDimBytes;
    }
    ...
    int32_t ndims_;
    int32_t transpose_ndims_;
    Storage storage_;
    int64_t* dims_;
    int64_t* transpose_dims_;
    int* transpose_minor_to_major_;
  };
