...
static void CopyCpuBufferToLiteral(const Shape& device_shape,
  ...
  if (can_use_zero_copy) {
    ...
    if (!has_default_layout) {
      ...
      if (should_sync_copy) {
        ...
        enqueue_work([device_buffer = std::move(device_buffer), dst_data_ptr,
...
void CopyLiteralToCpuBuffer(
  ...
  auto* device_buffer = cpu_buffer->AcquireUsage(std::move(usage_event));
  CHECK(device_buffer);
  if (!shape.IsTuple()) {
    ...
    for (int i = 0; i < shape.tuple_shapes_size(); ++i) {
      ...
      enqueue_work([i, literal, av = (*avs)[i].CopyRef(), shape,
...
PjRtFuture<Status> AbstractTfrtCpuBuffer::ToLiteralHelper(
    MutableLiteralBase* literal,
    absl::AnyInvocable<void(absl::Span<const tsl::RCReference<tsl::AsyncValue>>,
...
StatusOr<std::unique_ptr<TrackedTfrtCpuDeviceBuffer>>
AbstractTfrtCpuBuffer::CopyToDeviceHelper(
    absl::AnyInvocable<void(absl::AnyInvocable<void()>)> enqueue_work) {
  ...
  src_definition_event.AndThen([enqueue_work = std::move(enqueue_work),
                                copy_task = std::move(copy_task)]() mutable {
    enqueue_work(std::move(copy_task));
  });
