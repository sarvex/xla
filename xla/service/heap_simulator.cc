...
std::string GlobalDecreasingSizeBestFitHeap<
  ...
  return absl::StrCat(
      ...
      ", sorted_slice_sizes: { ", absl::StrJoin(sorted_slice_sizes, ", "),
      " }, sorted_slice_allocation_start_times: { ",
      absl::StrJoin(sorted_slice_allocation_start_times, ", "), " } }");
}
...
GlobalDecreasingSizeBestFitHeap<BufferType>::FindChunkCandidates(
  ...
  for (const BufferType* colocation :
       GetTransitiveColocations(sliced_buffer_interval.full_buffer_interval)) {
    max_colocation_size =
        std::max(max_colocation_size, buffer_intervals_.at(colocation).size);
  }

  // Prepare slicing data for each slice time, [0, num_slices).
  CHECK_EQ(sliced_buffer_interval.sorted_slice_sizes.size(),
           sliced_buffer_interval.sorted_slice_allocation_start_times.size());
  size_t num_slices =
      sliced_buffer_interval.sorted_slice_allocation_start_times.size();
  if (num_slices == 0) {
    // If no slices are specified, the full BufferInterval can be thought of as
    // as single slice.
    num_slices = 1;
  }
  // Since we don't yet know when we'll decide to fetch each slice, at slice
  // time i, we must be prepared to support the i largest slice sizes.
  std::vector<int64_t> sizes_sorted_by_size =
      sliced_buffer_interval.sorted_slice_sizes;
  if (sizes_sorted_by_size.empty()) {
    sizes_sorted_by_size.push_back(
        sliced_buffer_interval.full_buffer_interval.size);
  }
  std::sort(sizes_sorted_by_size.begin(), sizes_sorted_by_size.end(),
            std::greater<int64_t>());
  // Determine how much allocation time, each slice time will occupy. After
  // allocation_times is built, slice time i will correspond to allocation time
  // [times[i], times[i+1]].
  std::vector<int64_t> allocation_times =
      sliced_buffer_interval.sorted_slice_allocation_start_times;
  if (allocation_times.empty()) {
    allocation_times.push_back(
        sliced_buffer_interval.full_buffer_interval.start);
  }
  allocation_times.push_back(sliced_buffer_interval.full_buffer_interval.end);
  CHECK_EQ(sizes_sorted_by_size.size() + 1, allocation_times.size());

  // Build up a list of free chunks for each slice time.
  std::vector<FreeChunks> free_chunks_per_slice_time;
  free_chunks_per_slice_time.reserve(num_slices);
  int64_t min_required_free_chunk_size = 0;
  for (int slice_time = 0; slice_time < num_slices; ++slice_time) {
    min_required_free_chunk_size += sizes_sorted_by_size[slice_time];
    BufferInterval slice_interval{
        sliced_buffer_interval.full_buffer_interval.buffer,
        min_required_free_chunk_size,
        /*start=*/allocation_times[slice_time],
        /*end=*/allocation_times[slice_time + 1],
        /*colocations=*/{},
        sliced_buffer_interval.full_buffer_interval.need_allocation};
    ...
    free_chunks_per_slice_time.push_back(
        MakeFreeChunks(slice_interval, slice_interval_max_colocation_size));
  }

  std::vector<int64_t> sizes = sliced_buffer_interval.sorted_slice_sizes;
  if (sizes.empty()) {
    sizes.push_back(sliced_buffer_interval.full_buffer_interval.size);
  }

  auto chunks =
      SlicedAllocationFinder(free_chunks_per_slice_time, std::move(sizes),
                             max_colocation_size, preferred_offset, alignment_)
          .Find();
  if (num_slices == 1 && chunks.size() == 2) {
    // The second chunk is for colocations. Merge the 2 chunks, to reproduce
    // results in non-sliced cases.
    return {Chunk::FromOffsetSize(chunks[0].offset,
