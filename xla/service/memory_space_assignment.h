...
//
// To represent instructions, we keep track of three (previous, current, and
// next) iterations as well as the header and footer regions that are before and
// after the loop, respectively.
//
...
// than the producer, which would require 2X peak buffer consumption. This case
// is not currently supported yet.
//
...
class MemoryBoundLoopOptimizer {
 public:
  ...
  MemoryBoundLoopOptimizer(
      ...
      MemoryBoundLoopOptimizerOptions options,
      const HloLiveRange& hlo_live_range,
      const HloAliasAnalysis& alias_analysis_,
      const MemorySpaceAssignmentCostAnalysis& cost_analysis,
      const BufferValue::SizeFunction& size_function);

  // Initialized the data structures used by the optimizer. This method must be
  // called before calling Optimize.
  Status Initialize();
 ...
 private:
  // Sort LoopValues by savings_per_bytes.
  void SortLoopValues();
  ...
  // Returns the remaining bandwidth at the index.
  float GetRemainingBandwidth(int idx) const;

  // Returns the remaining bandwidth at the index assuming the given uses and
  // positions got alternate memory allocations.
  float GetRemainingBandwidth(
      int idx,
...
class AlternateMemoryBestFitHeap
 ...
 private:
  ...
  // For the given loop with, run the MemoryBoundLoopOptimizer and record its
  // outputs into optimized_allocations_map_.
  void OptimizeMemoryBoundLoop(int loop_start_idx, int loop_end_idx,
                               int loop_size_candidate);
  ...
  // Optimized allocations found by MemoryBoundLoopOptimizer and a map to look
  // up the optimized allocation by use.
  std::vector<MemorySpaceAssignment::AllocationSequence> optimized_allocations_;
  absl::flat_hash_map<
      HloUse,
      std::tuple<int64_t, int64_t, const MemorySpaceAssignment::Allocation*>>
