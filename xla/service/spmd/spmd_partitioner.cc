...
PartitionedHlo PartitionedHlo::ReshardWithAllToAll(
  ...
  auto temp_target_tile = sharding().tile_assignment().shared_array_clone();
  {
  ...
  auto temp_target = target.ReplicateOnLastTileDim()
                         ? HloSharding::PartialTile(std::move(temp_target_tile))
                         : HloSharding::Tile(std::move(temp_target_tile));
